...
async def register(user_data: UserRequest, db: AsyncSession = Depends(get_db)):
    ...
    response_data = UserRequest.UserAuthResponse(token=token, userInfo=UserRequest.UserInfoResponse.model_validate(user))
...
async def login(user_data: UserRequest, db: AsyncSession = Depends(get_db)):
    ...
    token = await users.create_token(db, user.id)
    response_data = UserRequest.UserAuthResponse(
        token=token,
        user_info=UserRequest.UserInfoResponse.model_validate(user)
    )
...
async def get_user_info(user: User = Depends(get_current_user)):
    return success_response(
        message="获取用户信息成功",
        data=UserRequest.UserInfoResponse.model_validate(user)
    )
